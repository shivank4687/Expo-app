...
export const fetchWishlistThunk = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            console.log('[wishlistSlice] Fetched items:', items);
            console.log('[wishlistSlice] Items count:', items?.length);
            return items;
        } catch (error: any) {
...
export const moveToCartThunk = createAsyncThunk(
    ...
    async ({ productId, quantity = 1 }: { productId: number; quantity?: number }, { rejectWithValue }) => {
        try {
            ...
            console.log('[wishlistSlice] Move successful, fetching updated wishlist...');
    ...
    extraReducers: (builder) => {
        ...
        builder
            ...
            .addCase(fetchWishlistThunk.fulfilled, (state, action) => {
                ...
                console.log('[wishlistSlice] State updated, items count:', state.items.length);
            })
        ...
        builder
            ...
            .addCase(moveToCartThunk.fulfilled, (state, action) => {
                console.log('[wishlistSlice] Move to cart fulfilled, updating items:', action.payload);
                state.items = action.payload;
                console.log('[wishlistSlice] State updated, items count:', state.items.length);
            })
